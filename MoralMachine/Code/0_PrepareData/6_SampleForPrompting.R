...
# Subset to US survey respondents who filled out the survey
SharedResponsesSurvey = fread("MoralMachine/Data/Awad2018/Data/Moral Machine Data/SharedResponsesSurvey.csv") %>% 
  mutate(Review_age = as.numeric(Review_age)) %>% 
  filter(UserCountry3 == "USA", 
         Review_age >= 18, Review_age <= 99,
         !is.na(Review_education), 
         !is.na(Review_gender), Review_gender!="default", 
         !is.na(Review_income), Review_income!="default", 
         !is.na(Review_political), 
         !is.na(Review_religious),
...
summarise_all(SharedResponsesSurvey, ~ sum(is.na(.))) 

# Check individual variable values
table(SharedResponsesSurvey$Saved,useNA = "always")
table(SharedResponsesSurvey$Review_age,useNA = "always")
table(SharedResponsesSurvey$Review_education,useNA = "always")
table(SharedResponsesSurvey$Review_gender,useNA = "always")
table(SharedResponsesSurvey$Review_income,useNA = "always")
table(SharedResponsesSurvey$Review_political,useNA = "always")
table(SharedResponsesSurvey$Review_religious,useNA = "always")

# get unique IDs in survey data
SurveyIDs = distinct(SharedResponsesSurvey,ResponseID)

# all IDs in survey dataset are also in the general dataset
all(SurveyIDs$ResponseID %in% SharedResponses$ResponseID)

# set target sample size
nobs = 15000

# sample responses by their IDs
set.seed(3337)
SurveyIDsToSample = slice_sample(SurveyIDs, n = nobs, replace=F) 

# Subset to random sample  
SurveyResponsesToSample = SharedResponsesSurvey %>% 
  filter(ResponseID %in% SurveyIDsToSample$ResponseID)

# Each response ID has two rows
all(count(SurveyResponsesToSample,ResponseID)$n == 2)

write_csv(SurveyResponsesToSample,"MoralMachine/Data/6_15000SampleWithSurvey.csv")


  
# Joined -----------------------------------------------------------------------  
  
# keep IDs that are in both 
IDsInSurveyAndJoined = semi_join(SurveyIDs,JoinedIDs,by="ResponseID")

JoinedSurveyResponsesToSample = SharedResponsesSurvey %>% 
  filter(ResponseID %in% IDsInSurveyAndJoined$ResponseID)

write_csv(SurveyResponsesToSample,"MoralMachine/Data/6_JoinedSampleWithSurvey.csv")
...
Saved = computeACME(data.table(SurveyResponsesToSample),"Saved")
Saved
pSaved = PlotAndSave(Saved$main, T, "MoralMachine/Figures/6_Saved_ResponsesWithSurvey", Saved$util,
                     paste0("Human respondents (US only, n=",nrow(SurveyResponsesToSample)/2,")"))
pSaved
