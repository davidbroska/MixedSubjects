...
JoinedLong <- fread(input="MoralMachine/Data/JoinedLong.csv")
...
Saved = computeACME(JoinedLong,"Saved")
SavedGPT3 = computeACME(JoinedLong,"SavedGPT3")
SavedGPT4 = computeACME(JoinedLong,"SavedGPT4")
SavedLlama2 = computeACME(JoinedLong,"SavedLlama2")
SavedPalm2 = computeACME(JoinedLong,"SavedPalm2")

pBarplot = JoinedLong %>% 
  select(contains("Saved")) %>% 
  summarise_all(~ sum(!is.na(.))) %>% 
  pivot_longer(cols = everything()) %>% 
  ggplot(aes(name,value)) + 
  geom_col(width = 0.5) + 
  theme_bw() +
  labs(x="Dataset",y="Number of observations",
       subtitle = "Number of observations")
pBarplot
# Joined data (the effect calculated on this subset of US respondents should be similar to the one reported in the Awad paper)
pSaved = PlotAndSave(Saved$main, T, "MoralMachine/Figures/5_Saved", Saved$util,"Human respondents (US only)")
pSavedGPT4 = PlotAndSave(SavedGPT4$main, T, "MoralMachine/Figures/5_SavedGPT4", SavedGPT4$util, "GPT4")
pSavedGPT3 = PlotAndSave(SavedGPT3$main, T, "MoralMachine/Figures/5_SavedGPT3", SavedGPT3$util, "GPT3")
pSavedLlama2 = PlotAndSave(SavedLlama2$main, T, "MoralMachine/Figures/5_SavedLlama2", SavedLlama2$util,"Llama2")
pSavedPalm2  = PlotAndSave(SavedPalm2$main, T, "MoralMachine/Figures/5_SavedPalm2", SavedPalm2$util, "Palm2")

library(cowplot)
plot_grid(pSaved,pBarplot,pSavedGPT4,pSavedGPT3,pSavedLlama2,pSavedPalm2, nrow = 3)

write_csv(distinct(JoinedLong,ResponseID),"MoralMachine/Data/5_ResponseID_JoinedLong.csv")
