...
corr_llms = left_join(llm_overview, corr, by = "LLM")
corr_llms
...
dvs = c("Saved","gpt4turbo_wp_Saved")
...
plot_results_ratio = function(.predictors, .n, .Nmax, .model, .rhos){
  ...
  dd = dfsim %>% 
    filter(x %in% .predictors, n ==.n, N <= .Nmax, 
           #x != "Social Status", 
  ...
  p1 = d1 %>% 
    ...
    geom_line() +
    labs(linetype="Language Model", y = "PPI CI width as percentage of classical CI",
  ...
  p3 = d3 %>% 
    ...
    geom_line() +
    geom_hline(yintercept = 95, color = "darkgrey", linetype = "dotted") +
    scale_x_continuous(labels = label_comma()) +
    scale_y_continuous(
