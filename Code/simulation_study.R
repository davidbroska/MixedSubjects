...
# Import averaged simulation results
dfe = read_csv("Data/simulation_study_rse.csv.gz")
...
# Create Bias Plot
bias_plot_e = dfe_long %>%
  filter(metric == "bias") %>%
  ggplot(aes(x = rse, y = value, color = method, linetype = method)) +
  geom_line(linewidth = linewidth) +
  geom_point(size = 1) +
  labs(
    title = " ",
    x = "Residual standard error Yhat=Y+e",
    y = "Bias"
  ) +
  scale_color_manual(values = custom_colors) +
  scale_linetype_manual(values = custom_linetypes, guide = "none") +
...
combined_plot = bias_plot_n + se_plot_n + coverage_plot_n + rmse_plot_n +
                ...
                bias_plot_e + se_plot_e + coverage_plot_e + rmse_plot_e +
